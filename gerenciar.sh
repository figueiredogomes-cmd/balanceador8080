...
# -------------------------------------------------------------------------
clear_cache() {
    echo "[🧹] Reiniciando topologia e limpando arquivos compartilhados..."
    ...
    # Inicializa os arquivos com 0 absoluto
...
# -------------------------------------------------------------------------
# CONFIGURAÇÃO DO BALANCEADOR CENTRAL (MÉTODO ROUND ROBIN)
# -------------------------------------------------------------------------
generate_configs() {
    cat << 'EOF' > $LB_CONF
...
http {
    ...
    upstream asr_cluster {
        # Encaminha as requisições igualmente entre as instâncias do cluster
    ...
    server {
        ...
        # Dashboard Principal do Monitor de Infraestrutura
        location / {
            root /usr/share/nginx/html;
            index index.html;
        }

        # Rota de processamento do Cluster ASR balanceado
        location /api {
        ...
        # Endpoint JSON que o front-end lê para renderizar os cartões
        location /stats.json {
...
EOF
    ...
    # Docker Compose estruturado com Node.js (Garante persistência de arquivos sem travamentos)
...
case "$1" in
    ...
    stop)
        ...
        echo "[✅] Instância $2 pausada. Simulando falha de hardware."
        ;;
    start)
        if [ -z "$2" ]; then echo "❌ Defina o nó: asr_node1, asr_node2 ou asr_node3"; exit 1; fi
        docker compose start $2
        echo "[✅] Instância $2 reintroduzida no cluster."
        ;;
    *)
